...
| `admin` | Agustín (jefe de ventas) · Fer · Fernando M Gonzalez · Diego Kusnetzoff · Juan Contini | Todo lo del vendedor + moderar tasaciones (rebotar o enviar a reventas) + ver precios de reventas en ranking + cerrar precio final · Único que ve CCA / Fórmula FMG / IA / diferencia $/% interna. **NO puede gestionar usuarios ni entrar como otros usuarios** |
...
| `supervisor` | Jefe de local: hoy **`martinp` (Martín Peralta, jefe de ventas de Obelisco)** con los 6 vendedores de Obelisco (`usuarios.supervisor_id` apunta a él) | **Solo mirar + cargar**: panel con las tasaciones de SUS vendedores (ve lo mismo que el vendedor, incl. monto del precio de toma; NO ve CCA/FMG/IA/precios de reventas/margen) + **cargar tasaciones en nombre de sus vendedores** (elige el vendedor en la barra del wizard; queda auditado en `tasaciones.cargada_por_id`). NO corrige rebotadas ni marca TOMADA — eso sigue siendo del vendedor/admin. Recibe **4** avisos WA reusando templates ya aprobados: `nueva_tasacion`, `tasacion_rebotada`, `precio_de_toma`, `usado_tomado` (eventos `sup_*` en el log; sin templates nuevos en Meta) — o sea, todo lo que le pasa a su local |
| `superadmin` (modo god) | **Los 4 dueños/directores, hardcodeados**: `fngonzalez` (Fer), `fgonzalez` (Fernando M Gonzalez), `diegok` (Diego Kusnetzoff), `juanc` (Juan Contini) | Todo lo del admin + alta/baja/edición de usuarios + reset de claves + cambiar el rol de cualquiera + **impersonation: entrar como cualquier otro usuario para ver lo que ve cada uno** |

**Importante:** `SUPERADMINS_USUARIOS` en `index.html` se toca SOLO desde el código — es a propósito (decisión de Fer 2026-08-19): los 4 pueden dar y sacar poder desde la plataforma (crear usuarios, activar/desactivar, cambiar rol) pero **nunca pueden hacer superadmin a nadie desde la UI** — el selector de Rol no ofrece "superadmin" y la lista no se edita por pantalla. Agustín NO es superadmin (sigue siendo el `admin` que opera todo el día). **Nunca mencionar "Agustín" en el copy de la UI** — usar "admin" o "jefe de ventas".

**Avisos masivos (`usuarios.notif_admin_bulk`, migration 017):** `false` = ese admin NO recibe `nueva_tasacion` ni `resumen_reventas`; sí sigue recibiendo los avisos personales de sus propias tasaciones. **ON**: `fngonzalez` (Fer) y `agustinc`. **OFF**: `fgonzalez`, `diegok`, `juanc` — los 3 directores pidieron no recibir un WhatsApp por cada cosa (decisión de Fer 2026-08-19). Martín no usa este flag: es `supervisor` y sus avisos salen por la vía `sup_*`, que ya filtra por sus vendedores (= solo Obelisco). Se maneja desde ⚙️ Configuración → Editar usuario (checkbox visible solo con rol Administrador).
...
- **Cierre directo con una reventa** (`cierreDirectoHTML` + `confirmarCierreDirecto`): para cuando una reventa ya pasó el precio por teléfono y la operación se cerró. Botón **⚡ Cierre directo** disponible en `pendiente_admin` (saltea el broadcast a las 7-8 reventas) **y** en `en_reventa`/`precios_recibidos` (una cerró por teléfono mientras estaba en reventas). El admin elige **1 reventa** + **precio que paga (ARS)** + **% de plataforma** (mismo selector editable) → calcula el precio de toma al cliente. Al confirmar: upsert en `reventas_precios` (ronda actual), comentario opcional a `comentarios_reventa`, y `PATCH tasaciones` a `precio_al_vendedor` con `reventa_ganadora_id` = esa reventa + `precio_final_admin` + `precio_al_vendedor_at`. Queda en la solapa **"Con precio"** lista para marcar **TOMADA en un clic** (reusa `abrirConfirmarToma` → `confirmarTomaFinal`, que setea `reventa_final_id` y avisa a la reventa). Así toda tasación queda en sistema aunque se resuelva rápido. Estado del form en `detalleActual`: `vistaCierreDirecto`, `cdDesc` (helpers `abrirCierreDirecto`/`cancelarCierreDirecto`/`_setCdDesc`/`_previewCierreDirecto`/`_resumenCierreInner`).
  - **Check "Pedir precio igual a todas las reventas"** (`cdPedirTodas`, prendido por defecto): aunque el precio al cliente ya quedó fijo, igual se les pide/avisa a todas (`notifyNuevaUnidadReventa` + setea `enviada_a_reventas_at` + re-arma el cron del resumen poniendo `resumen_reventas_enviado_at=null`). Las demás cotizan y sus precios entran como **POSTERIOR**; al **Confirmar con reventa** se le asigna la toma a la que **pague más** que la de referencia (más margen, mismo precio al cliente). Si se destilda, solo queda registrada esa reventa.
...
└── supabase/
    ...
    └── migrations/
        ...
        ├── 017_directores_y_jefe_obelisco.sql ← corrida 2026-08-19 (usuarios.notif_admin_bulk + martinp/diegok/juanc + los 6 de Obelisco a martinp + fn_resumen_reventas_pendientes filtra por el flag). Reemplaza a la 008, que NUNCA se había corrido — por eso _waAdmins() pegaba contra una columna inexistente y devolvía 400: ningún admin recibió jamás `nueva_tasacion` (0 en notificaciones_log). Arreglado acá
