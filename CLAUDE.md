...
**Avisos masivos (`usuarios.notif_admin_bulk`, migration 017):** `false` = ese admin NO recibe `nueva_tasacion` ni `resumen_reventas`; sí sigue recibiendo los avisos personales de sus propias tasaciones. Diego y Juan están en `false` (pidieron no recibir un WhatsApp por cada cosa). Se maneja desde ⚙️ Configuración → Editar usuario (checkbox visible solo con rol Administrador).
